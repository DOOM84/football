          ...
          <TheModal :show="showDlg" @close="closeModal" :showLoading="showLoading">
            <template #body>
              ...
              <div v-if="mode === 'edit'" class="flex justify-center">
                <img class="w-[30px] h-[30px]" :alt="cupTeamToUpdate.name" :title="cupTeamToUpdate.name"
                     src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVQYV2NgYAAAAAMAAWgmWQ0AAAAASUVORK5CYII="
                     :style="`background: url('/teams.png') ${cupTeamToUpdate.sprite};`"/>
              </div>

              <div class="my-3">
                <label for="title">Название</label>
                <input type="text" v-model.trim="cupTeamToUpdate.name" class="block
              ...
              <div class="my-3">
                ...
                <Multiselect
                    v-model="cupTeamToUpdate.team_id"
                    :options="data.teams"
                    :searchable="true"
                    valueProp="id"
                    label="name"
              ...
              <div class="my-3">
                ...
                <input type="text" v-model.trim="cupTeamToUpdate.api_id" class="block
              w-full h-8 px-2 py-3 text-md text-zinc-800 border
              ...
              <div class="my-3">
                ...
                <input type="text" v-model.trim="cupTeamToUpdate.sprite" class="block
              w-full h-8 px-2 py-3 text-md text-zinc-800 border
              ...
              <div class="text-right">
                ...
                <input type="checkbox" v-model="cupTeamToUpdate.status" id="status">
              </div>
              <button
                  type="button"
          ...
          <template #thead>
            <table-head>
              <div class="flex justify-center items-center">
                ...
                <template v-if="data.cupTeams && data.cupTeams.length">
...
import type {IError, ICupTeam, ITeam} from "~/types/interfaces";
...
const {data, pending} = useLazyFetch<{cupTeams: Partial<ICupTeam>[],
  teams: Partial<ITeam>[]}>('/api/admin/cupTeams')

useHead({
  titleTemplate: '%s - Команды (Кубки)'
})

const {filtering, toFilter, showDlg, mode, filter} = useFilter();

const initTeamData: Partial<ICupTeam> = {name: ''};

const cupTeamToUpdate = ref<typeof initTeamData>({});

const showLoading = ref<boolean>(false);

function closeModal(): void {
  cupTeamToUpdate.value = {...initTeamData};
  showDlg.value = false;
  mode.value = null;
}
...
function teamChanged(team: number) {
  ...
  (cupTeamToUpdate.value.team as Partial<ITeam>) = {...data.value?.teams[data.value.teams.findIndex(tm => tm.id === team)]}
  cupTeamToUpdate.value.team_id = team;
  cupTeamToUpdate.value.sprite = cupTeamToUpdate.value!.team!.sprite;
}

async function updateItem(cupTeam: ICupTeam) {
  mode.value = 'edit';

  cupTeamToUpdate.value = JSON.parse(JSON.stringify(cupTeam));
...
async function storeItem(): Promise<void> {
  ...
  try {
    ...
    const cupTeamToDb = {
      id: cupTeamToUpdate.value.id,
      api_id: +cupTeamToUpdate.value.api_id!,
      name: cupTeamToUpdate.value.name,
      sprite: cupTeamToUpdate.value.sprite,
      team_id: cupTeamToUpdate.value.team_id,
      status: cupTeamToUpdate.value.status,
    }

    if (mode.value === 'edit') {
      const {result} = await $fetch<{ result: ICupTeam }>('/api/admin/cupTeams/edit', {
        method: 'PUT',
        body: cupTeamToDb,
      })
      const ind: number = data.value?.cupTeams.findIndex((team: Partial<ICupTeam>) => team.id === cupTeamToUpdate.value.id) as number;

      if(data.value){
        data.value.cupTeams[ind] = {...cupTeamToUpdate.value};
      }

    } else if (mode.value === 'add') {
      const {result} = await $fetch<{ result: Partial<ICupTeam> }>('/api/admin/cupTeams/add', {
        method: 'POST',
        body: cupTeamToDb,
      })
      if(data.value){
        data.value.cupTeams.unshift({...cupTeamToUpdate.value, id: result.id});
      }
    }
    filter('', '');
    closeModal();
...
async function removeItem(dbId: number): Promise<void> {
  ...
  if (confirm('Are you sure?')) {
    try {
      ...
      const {id} = await $fetch<{ id: number }>('/api/admin/cupTeams/remove', {
        method: 'DELETE',
        body: {id: dbId},
      })

      if(data.value){
        data.value.cupTeams.splice(data.value.cupTeams.findIndex((item) => item.id === +id), 1);
      }
