  ...
  <tr class="border-b border-solid border-zinc-800 border-opacity-10 relative group" v-for="(result, i) in results"
    ...
    <td class="text-center res-logo">
      ...
      <div class="text-center py-1">
        <nuxt-link class="res-team hover:underline"
                   v-if="result.home.slug || result.home.team?.slug"
                   :to="'/team/'+(result.home.slug || result.home.team?.slug)">
          {{ result.home.name }}
        </nuxt-link>
      ...
      <transition name="page">
        ...
        >
          <ul class="basis-auto flex-grow: 0; overflow-x-hidden m-0 p-0">
            <li class="m-0 p-0" v-for="(goal, i) in (result as Record<string,any>).info[result.home.api_id]"
                :key="`home-${i}`">
              {{ htmlDec(goal.player.name) }} <small>{{ goal.time.elapsed }}'</small> <small
                v-if="goal.time.extra">(+{{ goal.time.extra }}')</small>
            </li>
          </ul>
          <ul class="basis-auto flex-grow: 0; overflow-x-hidden m-0 p-0 text-right">
            <li class="m-0 p-0 overflow-x-hidden"
                v-for="(goal, i) in (result as Record<string,any>).info[result.away.api_id]" :key="`away-${i}`">
              {{ htmlDec(goal.player.name) }} <small>{{ goal.time.elapsed }}'</small> <small
                v-if="goal.time.extra">(+{{ goal.time.extra }}')</small>
            </li>
          </ul>
    ...
    <td class="text-center res-logo py-1">
      ...
      <div class="text-center">
        <nuxt-link class="res-team hover:underline"
                   v-if="result.away.slug || result.away.team?.slug"
                   :to="'/team/'+(result.away.slug || result.away.team?.slug)">
          {{ result.away.name }}
        </nuxt-link>
        <span class="res-team" v-else>
            {{ result.away.name }}
...
async function setResult(res: IResult) {
  ...
  try {
    ...
    const {result, info} = await $fetch<{
      result?: boolean,
      info?: Record<any, any[]>
    }>(season!.value ? '/api/admin/setArchiveResult' : '/api/admin/setResult', {
      method: 'PUT',
